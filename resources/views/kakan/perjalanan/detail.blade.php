...
@section('content')
    @include('kakan.perjalanan.review')
    <div class="col-sm-12">
        ...
        <div class="col-md-6">
            <div class="card">
                    ...
                    <div class="btn-toolbar d-none d-md-block">
                        <div class="row">
                            @if($perjalanan->status == config('central.status.2'))
                                <div class="col-auto">
                                    <form action="{{ route('status.st.acc', $perjalanan->id) }}" method="post">
                                        @method('PATCH')
                                        @csrf
                                        <button class="btn form-control" type="submit"><i class="icon icon-lg far fa-check-square"></i> Setuju Surat Tugas</button>
                                    </form>
                                </div>

                            <div class="col-auto">

                                    <button class="btn form-control"  data-coreui-toggle="modal" data-coreui-target="#staticBackdropLive"><i class="icon icon-lg fas fa-times"></i> Tolak</button>
                            ...
                            @else
                                <div class="col-auto">
                                    <form action="{{ route('status.spd.acc.kk', $perjalanan->id) }}" method="post">
                                        @method('PATCH')
                                        @csrf
                                        <button class="btn form-control" type="submit"><i class="icon icon-lg far fa-check-square"></i> Setuju Surat Perjalanan Dinas</button>
                                    </form>
                                </div>
                            @endif
                        </div>
                ...
                <div class="card-footer">
                    <a href="{{ route('kakan.persetujuan') }}" class="btn btn-sm btn-dark align-middle"><i class="icon icon-lg fas fa-caret-left"></i> Back To Home</a>
                </div>
            </div>
        </div>
